...
const uploadComprobante = async () => {
  if (selectedPedidoId.value && selectedFile.value) {
    try {
      ...
      const uploadUrl = `https://apinodedvp.onrender.com/pedidosventas/${selectedPedidoId.value}`
...
// Computed property para filtrar los pedidos que no están aceptados
const pedidosFiltrados = computed(() => {
  return props.pedidos.filter((pedido) => pedido.EstadoVenta !== 'Aceptado')
})
</script>
<!-- eslint-disable prettier/prettier -->
  ...
  <VaModal v-model:modelValue="showModal" hide-default-actions @close="showModal = false">
    ...
    <form @submit.prevent="uploadComprobante">
      <div class="modal-body mt-5">
        ...
        <VaButton type="submit" preset="primary">Subir Comprobante</VaButton>
      </div>
    </form>
    <template #body> </template>
    <template #footer>
      <div class="modal-footer">
        <VaButton preset="secondary" @click="showModal = false">Cancelar</VaButton>
      </div>
    </template>
  </VaModal>
</template>
